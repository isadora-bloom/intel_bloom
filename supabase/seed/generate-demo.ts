  ...
  const rows = [
    {
      ...
      lost_deal_score: 78,
      alerts: JSON.stringify([{ type: "info", message: "Saturday tours fully booked 3 weeks out" }]),
      recommendations: JSON.stringify([{ action: "Add Sunday tour slots", impact: "high" }]),
    },
    {
      venue_id: VENUE.lakeview,
      calculated_at: fmtTs(addDays(NOW, -1)),
      ...
      alerts: JSON.stringify([
        { type: "critical", message: "Response time 6.2hrs — 3x above benchmark" },
        { type: "warning", message: "The Knot CPB up 34% in 90 days" },
      ]),
      recommendations: JSON.stringify([
        { action: "Implement 2-hour response SLA", impact: "critical" },
        { action: "Reduce The Knot spend, increase Google Ads", impact: "high" },
      ]),
    },
    {
      venue_id: VENUE.marchetti,
      ...
      lost_deal_score: 92,
      alerts: JSON.stringify([{ type: "info", message: "Zero Google Ads inquiries in 14 days — check account" }]),
      recommendations: JSON.stringify([{ action: "Test modest Google Ads spend ($300/mo)", impact: "medium" }]),
    },
    {
      venue_id: VENUE.cooperfield,
      calculated_at: fmtTs(addDays(NOW, -1)),
      ...
      lost_deal_score: 60,
      alerts: JSON.stringify([{ type: "info", message: "Only 14 reviews — below credibility threshold" }]),
      recommendations: JSON.stringify([{ action: "Prioritize review collection to reach 25+", impact: "high" }]),
    },
  ];
